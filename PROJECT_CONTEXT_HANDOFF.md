...
## Current Repo Reality (V2 branch)

`src/aegis/` contains only `__init__.py` — all v1 code wiped, clean slate for v2. V1 code is fully preserved on the `v1` branch.

**Git:**
- `v2` branch — active development (current)
- `v1` branch — frozen, all v1 code intact (88 tests, Billing RAG, Weather, AC Control)
- `master` — NEVER touch directly. Shared with internship company. Only updated via deliberate PR.

### V1 Package (frozen on `v1` branch)
